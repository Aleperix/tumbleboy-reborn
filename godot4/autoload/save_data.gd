...
# Port a Godot 4: File.new() -> FileAccess, parse_json -> JSON.parse_string,
# OS.get_unix_time -> Time.get_unix_time_from_system, JSON.print -> JSON.stringify.
...
func _save():
	...
	if file != null:
		file.store_string(JSON.stringify({ "version": SAVE_VERSION, "games": games, "downloaded_packs": downloaded_packs, "draft": draft, "settings": settings }))
		file.close()
...
func record_progress(completed: int):
	...
	s["updated"] = Time.get_unix_time_from_system()
