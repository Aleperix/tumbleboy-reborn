...
# Port a Godot 4: extends Reference -> RefCounted, Texture.get_data ->
# Texture2D.get_image, ImageTexture.new().create_from_image -> static
		...
		if ResourceLoader.exists(path):
			...
			if tex is Texture2D:
				if tex.get_size() != Vector2(size, size):
					...
					block_images.append(ImageTexture.create_from_image(img))
				else:
...
func render_board_image() -> Texture2D:
	load_block_images()
	...
	for y in range(height):
		for x in range(width):
			...
			if t >= 0 and t < block_images.size():
				var tile: Texture2D = block_images[t]
				if tile != null:
					var tile_img: Image = tile.get_image()
