...
# Port a Godot 4: extends Reference -> RefCounted, Directory -> DirAccess,
# File -> FileAccess, parse_json -> JSON.parse_string,
# ImageTexture.new().create_from_image -> ImageTexture.create_from_image,
# dir.remove -> DirAccess.remove_absolute.
...
static func extract_pack(zip_path: String, dest_dir: String) -> Array:
	...
	var out := []
	var dir := DirAccess.open(dest_dir.get_base_dir())
	if dir == null:
		return out
	dir.make_dir_recursive(dest_dir.get_file())
	for n in names:
...
static func get_thumbnail_texture(zip_path: String) -> Texture2D:
...
static func remove_pack(id: String) -> bool:
	if DirAccess.remove_absolute(PACKS_DIR + id + ".zip") == OK:
		return true
	return false
