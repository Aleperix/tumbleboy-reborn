...
# Port a Godot 4: File -> FileAccess, Directory -> DirAccess, parse_json ->
# JSON.parse_string, PoolStringArray -> PackedStringArray, PoolByteArray ->
# PackedByteArray, ImageTexture.new().create_from_image ->
# ImageTexture.create_from_image, HTTPRequest connect signals con Callable.
...
func refresh_index():
	...
	var headers := PackedStringArray(["Accept: application/vnd.github.raw+json"])
	if etag != "":
		headers.append("If-None-Match: " + etag)
	var err := http.request(API_INDEX_URL, headers, HTTPClient.METHOD_GET)
	if err != OK:
		_emit_cached_index()
...
func is_pack_installed(id: String) -> bool:
	return FileAccess.file_exists(PACKS_DIR + id + ".zip")
...
func _handle_index(result: int, response_code: int, headers: PackedStringArray, body: PackedByteArray):
...
func _handle_zip(result: int, response_code: int, body: PackedByteArray, id: String):
	if result != HTTPRequest.RESULT_SUCCESS or response_code != 200:
		emit_signal("pack_downloaded", id, false, "HTTP " + str(response_code))
		return
	var f := FileAccess.open(PACKS_DIR + id + ".zip", FileAccess.WRITE)
	if f == null:
...
func _handle_thumb(result: int, response_code: int, body: PackedByteArray, id: String):
	if result == HTTPRequest.RESULT_SUCCESS and response_code == 200:
		var f := FileAccess.open(CACHE_DIR + "thumbs/" + id + ".png", FileAccess.WRITE)
		if f != null:
			f.store_buffer(body)
			f.close()
	emit_signal("thumbnail_ready", id)
