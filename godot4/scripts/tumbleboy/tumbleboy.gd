...
# Port a Godot 4: change_scene -> change_scene_to_file, ev.scancode ->
# ev.keycode, Label.rect_position -> position, add_color_override ->
# add_theme_color_override, add_stylebox_override -> add_theme_stylebox_override,
# Directory -> DirAccess, TouchControlsScene.instance() -> .instantiate().
...
var welcome_image: Texture2D = null
var win_game_image: Texture2D = null
var menu_anim1: Texture2D = null
var menu_anim2: Texture2D = null
var win_anim1: Texture2D = null
var win_anim2: Texture2D = null
var good_job_image: Texture2D = null
...
func _ready():
	...
	level_label.position = Vector2(10 + C.GAME_OFFSET_X, 10)
	level_label.add_theme_color_override("font_color", Color(1, 1, 1))
	var sb := StyleBoxFlat.new()
	sb.bg_color = Color(0, 0, 0, 0.4)
	level_label.add_theme_stylebox_override("normal", sb)
...
func _unhandled_input(ev):
	if state == State.MENU:
		if ev is InputEventKey and ev.pressed and not ev.echo and not InputManager.is_back_key(ev.keycode):
			_start_playing()
		...
		if (ev is InputEventKey and ev.pressed and not ev.echo and not InputManager.is_back_key(ev.keycode)) or (ev is InputEventMouseButton and ev.pressed):
			_go_to_menu()
...
func _draw_menu():
	...
	var anim: Texture2D = menu_anim1 if anim_timer > 0.5 else menu_anim2
	if anim != null:
...
func _draw_win():
	...
	var anim: Texture2D = win_anim1 if anim_timer > 0.5 else win_anim2
	if anim != null:
...
func _draw_hint():
	...
	var text_size := hint_font.get_string_size(text, HORIZONTAL_ALIGNMENT_LEFT, -1, 20)
	...
	draw_string(hint_font, pos, text, -1, -1, 20, Color(1, 1, 1, 0.8))
