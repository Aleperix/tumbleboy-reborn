...
# str(raw).to_utf8() -> str(raw).to_utf8_buffer().
...
# files: { "ruta/dentro.zip": "contenido texto o PackedByteArray", ... }
static func write_pack_zip(zip_path: String, files: Dictionary) -> bool:
	...
	for name in files:
		var raw = files[name]
		var data: PackedByteArray = raw if raw is PackedByteArray else str(raw).to_utf8_buffer()
		var name_b: PackedByteArray = name.to_utf8_buffer()
		var crc := crc32(data)
	...
	f.store_buffer(PackedByteArray(cd))
