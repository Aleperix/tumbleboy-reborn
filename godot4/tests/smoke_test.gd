...
# Uso: godot --headless --path . res://scenes/SmokeTest.tscn
# Port a Godot 4: connect -> Callable, yield -> await, File -> FileAccess,
# Directory -> DirAccess, PoolByteArray -> PackedByteArray, get_focus_owner ->
# get_viewport().gui_get_focus_owner(), rect_min_size -> custom_minimum_size,
# Image.create como factoría estática, back de Android vía
# InputManager._notification(NOTIFICATION_WM_GO_BACK_REQUEST) (en headless no
# se propaga con get_tree().notification).
...
func _ready():
	...
	_test_android_back()
	await _test_list_layout()
	await _test_focus_reentry()
	_test_game_icon()
	_test_pack_row()
...
func _test_levels():
	print("== levels ==")
	var dir := DirAccess.open(C.LEVELS_DIR)
	var count := 0
	if dir != null:
		dir.list_dir_begin()
...
func _test_scene():
	print("== scene ==")
	var tb = TumbleBoyScene.instantiate()
	add_child(tb)
	tb._start_playing()
...
func _test_editor():
	...
	_check(FileAccess.file_exists("user://tumbleboy_levels/smoke_pack_level.txt"), "nivel guardado a user://")
	var zip_dir := DirAccess.open("user://tumbleboy_packs")
	if zip_dir != null:
	...
	ed._on_create_pack()
	_check(FileAccess.file_exists("user://tumbleboy_packs/smoke_pack.zip"), "pack ZIP creado")
...
func _backup_save() -> String:
	var f := FileAccess.open(SaveData.SAVE_PATH, FileAccess.READ)
	if f != null:
		var t := f.get_as_text()
		f.close()
		return t
...
func _restore_save(text: String):
	if text == "":
		...
		var f := FileAccess.open(SaveData.SAVE_PATH, FileAccess.WRITE)
		if f != null:
			f.store_string(text)
			f.close()
	SaveData._load()
...
func _test_story_hub():
	...
	var backup := _backup_save()
	SaveData.reset_all()
	var sh = StoryHubScene.instantiate()
	add_child(sh)
...
func _test_slot_picker():
	...
	var backup := _backup_save()
	SaveData.reset_all()
	var sp = SlotPickerScene.instantiate()
	add_child(sp)
...
func _test_packs_community():
	...
	var pc = PacksCommunityScene.instantiate()
	add_child(pc)
...
func _test_editor_hub():
	print("== editor hub ==")
	var eh = EditorHubScene.instantiate()
	add_child(eh)
	eh._on_open_niveles()
...
func _test_credits():
	print("== credits ==")
	var cr = CreditsScene.instantiate()
	add_child(cr)
...
func _test_save_validation():
	...
	_check(FileAccess.file_exists("user://tumbleboy_levels/smoke_validation.txt"), "guardar con campos completos funciona")
	DirAccess.remove_absolute(ProjectSettings.globalize_path("user://tumbleboy_levels/smoke_validation.txt"))
	ed.free()
...
func _test_thumbnail_zip():
	...
	var loaded_img := Image.create_empty(1, 1, false, Image.FORMAT_RGBA8)
...
func _test_list_layout():
	...
	var pc = PacksCommunityScene.instantiate()
	add_child(pc)
	pc.custom_minimum_size = Vector2(1200, 825)
	pc._on_open_descargados()
	var dh = await _wait_scroll_height(pc.desc_scroll)
	...
	pc._on_close_descargados()
	pc._on_open_online()
	var oh = await _wait_scroll_height(pc.online_scroll)
	...
	var eh = EditorHubScene.instantiate()
	add_child(eh)
	eh.custom_minimum_size = Vector2(1200, 825)
	eh._on_open_niveles()
	var nh = await _wait_scroll_height(eh.niveles_scroll)
	...
	eh._on_close_niveles()
	eh._on_open_packs()
	var ph = await _wait_scroll_height(eh.packs_scroll)
...
func _col_centered(col: Control) -> bool:
	...
	return abs(col.position.x - target) < 2.0
...
func _test_focus_reentry():
	...
	await get_tree().process_frame
	await get_tree().process_frame
	var dfo = pc.get_viewport().gui_get_focus_owner()
	_check(dfo != null and dfo.is_inside_tree(), "foco vivo al entrar en descargados")
	pc._on_close_descargados()
	pc._on_open_descargados()
	await get_tree().process_frame
	await get_tree().process_frame
	dfo = pc.get_viewport().gui_get_focus_owner()
...
func _test_game_icon():
	...
	_check(FileAccess.file_exists(icon_path), "archivo de icono existe")
...
func _test_pack_row():
	...
	_check(info_box is VBoxContainer and info_box.custom_minimum_size.x == 400, "info fija de 400px (got %s)" % str(info_box.custom_minimum_size))
	var preview: Control = row.get_child(0)
	_check(preview is TextureRect and preview.custom_minimum_size == Vector2(64, 64), "miniatura de 64px")
	...
	_check(play_btn.custom_minimum_size == Vector2(120, 36), "botones de acción a 120x36")
...
func _test_editor_ui_mode():
	...
	_check(ed.ui_controls[0].focus_neighbor_right == ed.ui_controls[1].get_path(), "toolbar cableada: Nuevo → Abrir")
	_check(ed.ui_controls[1].focus_neighbor_left == ed.ui_controls[0].get_path(), "toolbar cableada: Abrir → Nuevo")
	_check(ed.ui_fields.size() == 5, "ui_fields recoge los 5 campos editables (got %d)" % ed.ui_fields.size())
	_check(ed.name_edit.focus_neighbor_right == ed.author_edit.get_path(), "fila 2 cableada: nombre → autor")
	_check(ed.theme_option.focus_neighbor_left == ed.author_edit.get_path(), "fila 2 cableada: autor → tema")
	_check(ed.name_edit.focus_neighbor_top == ed.ui_controls[0].get_path(), "fila 1 → fila 2: nombre sube a Nuevo")
	_check(ed.ui_controls[0].focus_neighbor_bottom == ed.name_edit.get_path(), "fila 2 → fila 1: Nuevo baja a nombre")
	_check(ed.desc_edit.focus_neighbor_top == ed.name_edit.get_path(), "fila 3 → fila 2: descripción sube a nombre")
	_check(ed.name_edit.focus_neighbor_bottom == ed.desc_edit.get_path(), "fila 2 → fila 3: nombre baja a descripción")
	var first_palette: Control = ed.ui_controls[ed.toolbar_controls.size()]
	_check(first_palette.focus_neighbor_top == ed.desc_edit.get_path(), "paleta → fila 3: primer bloque sube a descripción")
	_check(ed.desc_edit.focus_neighbor_bottom == first_palette.get_path(), "fila 3 → paleta: descripción baja al primer bloque")
	ed.free()
...
func _test_android_back():
	...
	InputManager._notification(Node.NOTIFICATION_WM_GO_BACK_REQUEST)
...
func _count_wired(root: Node, scroll: ScrollContainer) -> int:
	...
	for ch in root.get_children():
		if ch is Button:
			for c in ch.focus_entered.get_connections():
				var cb: Callable = c.get("callable")
				if cb != null and cb.get_object() == scroll and cb.get_method() == "ensure_control_visible":
					count += 1
...
func _test_focus():
	...
	_check(mm.get_viewport().gui_get_focus_owner() != null, "MainMenu toma el foco al arrancar")
	navs.append(mm)
	var sp = SlotPickerScene.instantiate()
	get_tree().root.add_child(sp)
	_check(sp.get_viewport().gui_get_focus_owner() != null, "SlotPicker toma el foco al arrancar")
	_check(sp.get_viewport().gui_get_focus_owner() is Button, "foco inicial del SlotPicker en un botón")
	navs.append(sp)
	var pc = PacksCommunityScene.instantiate()
	get_tree().root.add_child(pc)
	_check(pc.get_viewport().gui_get_focus_owner() != null, "PacksCommunity toma el foco al arrancar")
	navs.append(pc)
	var eh = EditorHubScene.instantiate()
	get_tree().root.add_child(eh)
	_check(eh.get_viewport().gui_get_focus_owner() != null, "EditorHub toma el foco al arrancar")
	navs.append(eh)
	var cr = CreditsScene.instantiate()
	get_tree().root.add_child(cr)
	_check(cr.get_viewport().gui_get_focus_owner() != null, "Credits toma el foco al arrancar")
	navs.append(cr)
	for n in navs:
		n.free()
	NavParams.clear()
