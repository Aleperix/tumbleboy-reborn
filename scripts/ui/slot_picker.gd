...
func _build_ui():
	...
	sub_label = Label.new()
	sub_label.text = _mode_label()
	sub_label.add_font_override("font", UIFonts.make_font(15))
	sub_label.add_color_override("font_color", Color(0.7, 0.67, 0.8))
	sub_label.align = Label.ALIGN_CENTER
	...
	for i in SaveData.SLOT_COUNT:
		...
		btn.rect_min_size = Vector2(460, 50)
		btn.add_font_override("font", UIFonts.make_font(16))
		btn.add_color_override("font_color", Color(0.95, 0.93, 0.88))
		btn.text = "Zócalo %d — Vacío" % (i + 1)
		btn.connect("pressed", self, "_on_slot", [i])
		col.add_child(btn)
		slot_buttons.append(btn)

	if intent == "new":
		var spacer2 := Control.new()
		spacer2.rect_min_size = Vector2(0, 6)
		...
		no_save_button.rect_min_size = Vector2(460, 50)
		no_save_button.add_font_override("font", UIFonts.make_font(16))
		no_save_button.add_color_override("font_color", Color(0.8, 0.8, 0.7))
		no_save_button.connect("pressed", self, "_on_no_save")
		col.add_child(no_save_button)

	var spacer3 := Control.new()
	spacer3.rect_min_size = Vector2(0, 16)
